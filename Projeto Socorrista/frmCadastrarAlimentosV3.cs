...
namespace Projeto_Socorrista
{
    public partial class frmControleDeAlimentosV3 : Form
    {
        ...
        private void carregaDoacoes()
        {
            ...
            string sql = "SELECT DATE_FORMAT(prod.dataArrecadacao, '%d/%m/%Y'), DATE_FORMAT(prod.dataDeValidade, '%d/%m/%Y'), prod.nome, prod.quantidade, prod.unidadeMedida, prod.peso, usr.email FROM tbprodutos as prod INNER JOIN tbusuarios AS usr ON prod.codUsu = usr.codUsu ORDER BY dataArrecadacao DESC LIMIT 5;";
